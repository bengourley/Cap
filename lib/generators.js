...
var createGenerators = function (parser, errorReporter) {
  ...
  generators['assignment'] = function (node, meta) {
    ...
    var symbol = node.childNodes[0].childNodes[0].value;

    var output = meta.scope.indexOf(symbol) === -1 && meta.omitReturn ? 'var ' : '';
    ...
    exprmeta.scope.push(symbol);
    exprmeta.assignSymbol = output;
  ...
  generators['functionLiteral'] = function (node, meta) {
    ...
    var output = 'function ' + (meta.assignSymbol || '__anon') +
