...
var createGenerators = function (filename, errorReporter) {
  ...
  generators['program'] = function (node) {
    ...
    var output = '(function (module, exports) {\n';

    // Proxy to the `require()` function appending
    // `.cap.js` file extension
    output += 'var _require = require;\n';
    output += 'require = function (module) { return _require(module + \'.cap.js\'); };\n';
    // To simulate `require()` module loading system in
    // the browser, the file should know its own location
    output += generate(node.childNodes[0], meta);
    output += '}(typeof module !== \'undefined\' ? module : _module(\'' + filename + '\'),\n' +
      'typeof exports !== \'undefined\' ? exports : _exports(\'' + filename + '\')));';
    return output;
  ...
  generators['assignment'] = function (node, meta) {
    ...
    var output = (meta.scope.indexOf(symbol.value) === -1 &&
                    meta.parentNode !== 'objectLiteral') ? 'var ' : '';
    ...
    exprmeta.scope = exprmeta.scope.concat([symbol.value]);
    ...
    if (!meta.omitReturn)
      output += ';\nreturn ' + generate(symbol, newmeta);
