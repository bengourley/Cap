...
var createGenerators = function (parser, errorReporter) {
  ...
  generators['assignment'] = function (node, meta) {
    ...
    var output = 'var ' + generate(node.childNodes[0], newmeta);
    var newnewmeta = Object.create(meta);
    newnewmeta.scope.push(node.childNodes[0].childNodes[0].value);
    newnewmeta.assignSymbol = output;
    output += meta.objectLiteral ? ':' : '=';
    output += generate(node.childNodes[1], newnewmeta);
    return output;
  };
  ...
  generators['functionLiteral'] = function (node, meta) {
    ...
    var output = 'function ' + (meta.assignSymbol || '_anonymousfn') +
