...
var createGenerators = function (filename, errorReporter) {
  ...
  generators['call'] = function (node, meta) {
    ...
    var reflength = node.childNodes[0].childNodes.length;
    if (reflength > 1) {
      output += generate(node.childNodes[0].childNodes[reflength - 2], meta);
    } else {
      output += 'null';
    }

    if (node.childNodes[1].type !== 'tuple' ||
          node.childNodes[1].childNodes[0].type !== 'leaf' ||
