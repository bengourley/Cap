
// Module dependencies.
var fs = require('fs'),
	token = require('./token');

// `Lexer` constructor
var Lexer = module.exports = function () {
	this.input = '';
	this.completeInput = '';
	this.indentStack = [''];
	this.tokens = [];
	this.tokenStash = [];
	this.yytext = null;
	this.yylineno = 0;
	this.position = 0;
	this.startOfFile = true;
};

// List of reserved words
var ReservedWords = [
	'if', 'else', 'where'
];

// ## Lexer prototype
// Augment the `Lexer`'s prototype
// with the following methods.
Lexer.prototype = {

	// Runs the lexical analysis and returns tokens.
	// This implements the basic scanner interface that
	// jison requires.
	lex : function () {

		// Deal with the special case of whitespace at the
		// start of a file, which would otherwise be consumed
		// without error.
		if (this.startOfFile) {
			if (/^[ \t]+/.exec(this.input)) {
				this.tokenStash.push(token({ type : 'error', value : 'File starts with an indent' }));
			}
			this.startOfFile = false;
		}

		// Set the token to return. Check in the `tokenStash`
		// before reading in new input, in case more than one
		// token was generate in a previous pass.
		var t = this.tokenStash.length > 0
			? this.tokenStash.shift()
			: this.next();

		// If the next token is actually an array of tokens,
		// store them in the `tokenStash`.
		if (Array.isArray(t)) {
			this.tokenStash = this.tokenStash.concat(t);
			t = this.tokenStash.shift();
		}

		/*console.log(token);*/

		// Set `yytext` to the token's value. This is a shared
		// variable that the generated parser looks in for the
		// value of the current token.
		this.yytext = t.value || null;

		// Return just the token's type.
		return t.type;
		
	},
	
	// Sets the input of the `Lexer` to
	// the given string.
	setInput : function (str) {
		this.input = str;
		this.completeInput = str;
	},

	// Returns the next 20 characters of
	// input for useful error messages in
	// the generated parser.
	/*showPosition : function () {
		return '----------------------------------------\n' +
			this.completeInput.substr(this.position - 5, this.position + 5) +
			'\n----------------------------------------';
	},*/

	// Discards `num` characters
	// from the start of the input.
	advance : function (num) {
		this.input = this.input.substr(num);
		this.position += num;
	},

	// Retrieves the next token recognised on the input.
	next : function () {

		// Each of these methods only return a truthy value
		// if they return a token. The order of these functions
		// matter, since `linearWhitespace()` and `newline()` can
		// consume input without returning anything.
		// A call to `single()` will always return a token, which
		// will be an allowed single character of the language, or
		// an 'unrecognised character' error token.

		return this.linearWhitespace()
			|| this.newline()
			|| this.eof()
			|| this.literal()
			|| this.identifier()
			|| this.number()
			|| this.string()
			|| this.single();

	},

	// ### Recogniser methods:
	
	// Recognises end of input
	eof : function () {
		if (this.input.length === 0) {
			return token({ type : '$end'});
		}
	},

	// Recognises linear whitespace: ` ` and `\t`

	linearWhitespace : function () {
		var res = /^[ \t]+/.exec(this.input);
		if (res) {
			this.advance(res[0].length);
		}
	},

	// Recognises a newline character and
	// sends off the following whitespace
	// to be analysed for indents.
	newline : function () {
		var res = /^[\n\r\u2028\u2029]+([\t ]*)/.exec(this.input), indent;
		if (res) {
			var tokens = [token({ type : 'vwhitespace'})];
			this.advance(res[0].length);
			this.yylineno += /^[\n\r\u2028\u2029]+/.exec(res[0])[0].length;
			indent = res[1];
			// Determine indent
			if (indent !== this.indentStack[this.indentStack.length - 1]) {
				tokens = this.determineIndent(indent);
			}
			return tokens;
		}
	},

	 // Analyses a change in indent and produces
	 // a token if required, which may turn out
	 // to be an indent, a newline or an error
	 // (inconsistent indentation).
	determineIndent : function (indentString) {

		if (indentString.indexOf(this.indentStack.join('')) === 0) {
			this.indentStack.push(indentString);
			return [token({ type : 'vwhitespace' }), token({ type : 'indent' }), token({ type : 'vwhitespace' })];
		} else {

			var dedentCount = 0;
			while (this.indentStack.join('') !== indentString && this.indentStack.pop()) {
				dedentCount++;
			}

			if (this.indentStack.join('') !== indentString) {
				return token({ type : 'error', value : 'Inconsistent indentation' });
			} else {
				var dedents = [token({ type : 'vwhitespace' })];
				for (var i = 0; i < dedentCount; i++) {
					dedents.push(token({ type : 'dedent'}), token({ type : 'vwhitespace' }));
				}
				return dedents;
			}

		}

	},

	// Recognises a literal: `object:`, `function:` or `array:`
	literal : function () {
		var res = /^(object|function|array):/.exec(this.input);
		if (res) {
			this.advance(res[0].length);
			return token({ type : res[1] + 'literal' });
		}
	},

	 // Recognises an identifier eg. `foo`, `bar`.
	 // Also recognised reserved words by matching
	 // against the whitelist of reserved words.
	identifier : function () {
		var res = /^([A-Z|a-z]+)[\s\S]/.exec(this.input);
		if (res) {
			this.advance(res[1].length);
			if (ReservedWords.indexOf(res[1]) === -1) {
				return token({ type : 'identifier', value : res[1] });
			} else {
				return token({ type : res[1] });
			}
		}
	},

	// Recognises a number eg. `12`, `3.142`
	number : function () {
		var res = /^\d+(\.?\d+)?/.exec(this.input);
		if (res) {
			this.advance(res[0].length);
			return token({ type : 'number', value : res[0] });
		}
	},

	 // Recognises a string. e.g `'Hello, World!'`
	string : function () {
		var res = /^'([^'])*'/.exec(this.input);
		if (res) {
			this.advance(res[0].length);
			return token({ type : 'string', value : res[0] });
		}
	},

	 // Recognises any allowed single character.
	 // If this fails, create an error token
	 // with the offending character.
	single : function () {
		
		var c = this.input.substr(0, 1);
		this.advance(1);

		switch (c) {
		case '=': return token({ type : 'equals' }); break;
		case '.': return token({ type : 'dot' }); break;
		case '{': return token({ type : 'leftbrace' }); break;
		case '}': return token({ type : 'rightbrace' }); break;
		case '[': return token({ type : 'leftsquarebracket' }); break;
		case ']': return token({ type : 'rightsquarebracket' }); break;
		case '"': return token({ type : 'speechmark' }); break;
		case '~': return token({ type : 'tilde' }); break;
		case '+': return token({ type : 'plus' }); break;
		case '(': return token({ type : 'leftbracket' }); break;
		case ')': return token({ type : 'rightbracket' }); break;
		case ',': return token({ type : 'comma' }); break;
		default: return token({ type : 'error', value : 'Unrecognized character: ' + c });
		}

	}

};

/*fs.readFile('example.cap', 'utf8', function (err, data) {
	if (err) {
		throw err;
	} else {
		var l = new Lexer();
		l.setInput(data);
		var t = {};

		t = l.lex();
		console.log(t);
		while (t !== '$end') {
			t = l.lex();
			console.log(t);
		}
	}
});*/
