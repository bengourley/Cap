...
var lexer = function (spec) {
	...
	// Expose `yytext` and `yylineno` to the parser
	lexer.yytext = null;
	lexer.yylineno = 0;
	...
	literal = function () {
		var res = /^(object|function|array):/.exec(input);
		if (res) {
			advance(res[0].length);
	...
	single = function () {
		...
		switch (c) {
		case '=': return token({ type : 'equals' });
		case '.': return token({ type : 'dot' });
		case '{': return token({ type : 'leftbrace' });
		case '}': return token({ type : 'rightbrace' });
		case '[': return token({ type : 'leftsquarebracket' });
		case ']': return token({ type : 'rightsquarebracket' });
		case '"': return token({ type : 'speechmark' });
		case '~': return token({ type : 'tilde' });
		case '+': return token({ type : 'plus' });
		case '(': return token({ type : 'leftbracket' });
		case ')': return token({ type : 'rightbracket' });
		case ',': return token({ type : 'comma' });
		case '*': return token({ type : 'asterisk' });
		case '/': return token({ type : 'forwardslash' });
	...
	//lexer.showPosition = function () { return '??' };
