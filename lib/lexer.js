...
var createLexer = function (spec) {
  ...
  var location = function () {
    ...
    while (!finished) {
      if (start === 0 || foundLines === lines) {
        finished = true;
      } else {
        start--;
        if (completeInput.charAt(start) === '\n') {
          foundLines++;
        }
      }
    }
    ...
    while (!finished) {
      if (end >= completeInput.length - 1 || foundLines === lines) {
        finished = true;
      } else {
        end++;
        if (completeInput.charAt(end) === '\n') {
          foundLines++;
        }
      }
    }
