	...
	return {
		...
		StatementList : function (statements) {
			...
			this.compile = function () {
				var code = '';
				this.statements.forEach(function (s) {
					code += indentHandler.getIndent() + s.compile() + (s.type !== 'Conditional' ? ';\n' : '\n');
				});
				return code;
			};
		...
		Assign : function (variable, to) {
			...
			this.compile = function () {
				return 'var ' + this.variable.compile() + ' = ' + this.to.compile();
			};
		...
		Call : function (identifier, args) {
			...
			this.compile = function () {
				...
				return this.identifier.compile() + '(' + args.substr(0, args.length - 2) + ')';
			};
