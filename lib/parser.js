...
var createParser = function () {
  ...
  var statement = function () {
    ...
    if (expr) {
      ...
      if (expr.type === 'call' && lookahead(1).type === 'where') {
        node = where(expr);
        accept('vwhitespace');
        return node;
  ...
  var number = function () {
    ...
    if (t.type === 'number') {
      next();
      return node({ value : t.value, lineno : lexer.lineno });
    } else {
      return false;
    }
  };
  ...
  var bool = function () {
    ...
    if (t.type === 'true' || t.type === 'false') {
      next();
      return node({ value : t.type, lineno : lexer.lineno });
    } else {
      return false;
    }
  };
  ...
  var parseError = function (msg) {
    ...
    var ParseError = new Error(
      'Parse error on line ' + lexer.lineno +
      ' - ' + msg + parser.location()
    );
