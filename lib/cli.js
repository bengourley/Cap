      ...
      collectFilenames('./', function (err, capfiles) {
        ...
        async.map(capfiles, function (capfile, callback) {
          fs.readFile(capfile, function (err, data) {
            if (err) callback(err);
            callback(null, { filename : capfile + '.js', data : data });
          });
          ...
          async.map(sources, compile, function (err, out) {
            if (err) {
              ...
              if (!program.browser) {
                ...
                fs.readFile(__dirname + '/runtime/browser.js', function (err, browser) {
                  if (err) throw err;
                  output += browser;
                  out.forEach(function (file) {
                    output += file.output;
                  });
                  var name = program.args[0] ? program.args[0] + '.cap' : 'browser-build.cap';
                  write({
                    name : name,
                    output : output
                  }, function (err) {
